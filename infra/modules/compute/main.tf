...
# ---------------------------------------------------------------------------
# Event Source Mapping — Notifications SQS -> Async Consumer Lambda
# ---------------------------------------------------------------------------
resource "aws_lambda_event_source_mapping" "notifications_sqs_to_consumer" {
  count = var.enable_async ? 1 : 0

  event_source_arn = var.sqs_notifications_queue_arn
  function_name    = aws_lambda_function.async_consumer[0].arn
